"""Maze attention-extraction helpers used by generate_bar_data.py.
Conventions vs. Sudoku:
  - Grid is 30x30 (900 cells), not 9x9 (81).
  - Violation is "decoded z_H != ground-truth label" (both false-o and missed-o).
  - Three neighborhood definitions are reported (4-conn, 8-conn, 5x5 window).
  - Maze inner forward has NO input_embeddings re-injection, so decode_stage_grids
    is rewritten to match the checkpoint's f_level(z, z') signature.
"""
...
from models.layers import apply_rotary_pos_emb  # noqa: E402
from pretrain import PretrainConfig, create_dataloader, init_train_state  # noqa: E402


@dataclass
class AttentionContext:
    """Loaded runtime context for attention extraction (Maze)."""
    checkpoint_path: str
...
def resolve_checkpoint_path(path: str) -> str:
    """Resolve a checkpoint path. If `path` is a directory, return the latest step_<N> file."""
...
def load_checkpoint_and_test_loader(
    ...
    *,
    ...
    """Load config/checkpoint/model and create the test loader."""
    ...
    eval_loader, eval_metadata = create_dataloader(
        ...
        epochs_per_iter=1, global_batch_size=eval_batch_size,
    )
...
def build_inner_call_schedule(h_cycles: int, l_cycles: int) -> List[Tuple[str, int, Optional[int]]]:
    """Mirror the maze inner forward's call order.
      ...
      for H in range(H_cycles):
        for L in range(L_cycles):
          if not ((H == H_cycles-1) and (L == L_cycles-1)):
            z_L = f(z_L, z_H)   # ("L", H, L)
        if H != H_cycles-1:
          z_H = f(z_H, z_L)     # ("H", H, None)
      z_L = f(z_L, z_H)         # final L  ("L", H_cycles-1, L_cycles-1)
      z_H = f(z_H, z_L)         # final H  ("H", H_cycles-1, None)
    """
...
def _compute_maps_for_heads(
    *,
    ...
    """Compute post-softmax attention maps for selected heads at one layer call.

    Computes the puzzle-token attention sub-block.
    Returns the
    (NUM_CELLS, NUM_CELLS) puzzle-token sub-block instead of the 81x81 one.
    """
...
def capture_lh_maps_all_steps_maze(
    *,
    ...
    """Capture stage-targeted L/H attention maps for a maze checkpoint.

    Mirrors the Sudoku-side capture but does not
    assume the Sudoku inner forward (which adds input_embeddings). Here the
    schedule uses build_inner_call_schedule above which matches the maze model.

    Returns out[act_step][head]["L"|"H"] = (NUM_CELLS, NUM_CELLS) attention map.
    """
    ...
    if target_l not in schedule or target_h not in schedule:
        raise ValueError("Invalid stage targets for this model config.")
...
def decode_stage_grids_all_steps_maze(
    *,
    air_model: torch.nn.Module,
    batch: Dict[str, torch.Tensor],
    max_act_steps: int,
    ...
    """Replicate the default air_1net_Lx_H inner forward for Maze.

    Matches the standard "one x in L, no x in H" pattern:
      z_L = f(z_L, z_H + input_embeddings)
      z_H = f(z_H, z_L)
    Decodes to (GRID_SIZE, GRID_SIZE) grids.
    """
    inner = air_model.inner
    ...
    def decode_grid(z: torch.Tensor) -> np.ndarray:
        ...
        pred = torch.argmax(logits, dim=-1)[0].detach().cpu().numpy().reshape(GRID_SIZE, GRID_SIZE)
