...
CKPT_PATH="checkpoints/Maze/air_Lx_H_lr1e4_bs768_20k_epochs_run1/step_26040"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Generating Maze attention bar-chart data"
...
OMP_NUM_THREADS=8 python "$SCRIPT_DIR/generate_bar_data.py" \
  ...
  --num-puzzles 1000
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Completed Maze attention bar-chart data generation"
