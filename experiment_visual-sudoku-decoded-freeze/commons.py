"""Shared setup and tracing for Sudoku visual freeze/decode experiments."""
...
from models.air.air_1net_Lx_H import (
    AIR_Inner,
    AIRInnerCarry,
)

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# Built Sudoku dataset uses arr+1 (build_sudoku_dataset.py): raw 0->1 (blank), 1..9->2..10. Pad=0.
BLANK_TOKEN = 1
DEFAULT_NUM_QUESTIONS = 1000
DEFAULT_CHECKPOINT_DIR = os.path.join(
    AIR_ROOT, "checkpoints", "Sudoku", "air_Lx_H_lr1e4_bs768_20k_epochs"
)
DEFAULT_CHECKPOINT_FILE = os.path.join(DEFAULT_CHECKPOINT_DIR, "step_23436")


def resolve_checkpoint_paths() -> tuple[str, str]:
    """Resolve checkpoint and config paths, allowing override via environment."""
    checkpoint_file = os.environ.get("AIR_SUDOKU_CKPT_PATH", DEFAULT_CHECKPOINT_FILE)
...
def apply_sudoku_l_update(
    ...
    """Apply the architecture-specific L update used by the loaded Sudoku variant."""
    ...
    if module_name == "air_1net_input_scaling":
        return inner.f_level(z_L, z_H + inner.config.alpha * input_embeddings, **seq_info)

    if module_name == "air_1net_minus_x":
        return inner.f_level(z_L, z_H - input_embeddings, **seq_info)

    if module_name == "air_1net_gx_linear":
        return inner.f_level(z_L, z_H + inner.g_input(input_embeddings), **seq_info)

    if module_name == "air_1net_gx_non_linear":
        return inner.f_level(z_L, z_H + torch.tanh(inner.g_input(input_embeddings)), **seq_info)

    if module_name == "air_1net_had_prod":
        return inner.f_level(z_L, (z_H + z_L) * input_embeddings, **seq_info)

    if module_name == "air_1net_L2x_Hx":
        return inner.f_level(z_L + input_embeddings, z_H + input_embeddings, **seq_info)

    raise NotImplementedError(
        f"Sudoku tracing does not yet support the variant update rule for module '{module_name}'."
    )


def apply_sudoku_h_update(
    inner: AIR_Inner,
    z_H: torch.Tensor,
    z_L: torch.Tensor,
    input_embeddings: torch.Tensor,
    ...
    """Apply the architecture-specific H update used by the loaded Sudoku variant."""
    module_name = inner.__class__.__module__.rsplit(".", 1)[-1]
...
def run_inner_with_tracing(
    ...
    *,
    ...
    forbid_blank_in_zL: bool = False,
):
    """Run inner loop step-by-step. Optionally freeze z_H, freeze z_L, or forbid blank in z_L decode."""
    ...
    def decode(z, forbid_blank: bool = False):
        logits = inner.lm_head(z)[:, plen:].float().cpu()
        if forbid_blank and logits.shape[-1] > BLANK_TOKEN:
            logits[..., BLANK_TOKEN] = float("-inf")
        ...
        for H_step in range(H_cycles):
            for L_step in range(L_cycles):
                if not ((H_step == H_cycles - 1) and (L_step == L_cycles - 1)):
                    if not freeze_z_L:
                        z_L = apply_sudoku_l_update(inner, z_L, z_H, input_embeddings, seq_info)
                    traces.append(
                        {
                            "stage": "L",
                            "H_step": H_step,
                            "L_step": L_step,
                            "pred_H": decode(z_H).reshape(-1, 9, 9),
                            "pred_L": decode(z_L, forbid_blank=forbid_blank_in_zL).reshape(-1, 9, 9),
                        }
                    )
            if not (H_step == H_cycles - 1):
                if not freeze_z_H:
                    z_H = apply_sudoku_h_update(inner, z_H, z_L, input_embeddings, seq_info)
                traces.append(
                    {
                        "stage": "H",
                        "H_step": H_step,
...
def run_act_with_tracing(
    ...
    *,
    ...
    """Run the full ACT loop with tracing for a single batch."""
    ...
    with torch.inference_mode():
        for act_step in range(halt_max_steps):
            ...
            traces, new_inner_carry, logits, _, _ = run_inner_with_tracing(
                ...
                forbid_blank_in_zL=forbid_blank_in_zL,
            )
...
def load_config_model_batch(batch_index: int = 0):
    """Load config, model, one batch, and run the full ACT loop with tracing."""
    ...
    act_step_traces = run_act_with_tracing(air, inner, batch)
...
def load_config_model_batch_with_options(
    ...
    """Load config, model, one batch, and run tracing with the requested options."""
    config, air, inner, eval_loader = _load_config_model_and_eval_loader()
    eval_iter = iter(eval_loader)
    for _ in range(batch_index + 1):
        _set_name, batch, _ = next(eval_iter)
...
def load_config_model_batches(
    ...
    *,
    ...
    """Load config/model once, then run tracing over the first num_questions eval puzzles."""
    config, air, inner, eval_loader = _load_config_model_and_eval_loader()
    eval_iter = iter(eval_loader)
    question_runs = []
...
def batch_grid(batch: dict, key: str) -> np.ndarray:
    """Return a (9, 9) numpy grid from a batch tensor field."""
    return batch[key].detach().cpu().numpy().reshape(-1, 9, 9)[0]
