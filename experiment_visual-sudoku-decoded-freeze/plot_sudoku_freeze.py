#!/usr/bin/env python3
"""Plot Sudoku freeze experiment JSON outputs."""
...
def plot_result_json(input_path: Path, data: dict, output_path: Path):
    ...
    total_normal = summary_row(summary_table, "Total number changes: normal")
    total_freeze = summary_row(summary_table, f"Total number changes: freeze {freeze_axis}")
    ...
    axes[0].set_ylabel("Total number changes")
    axes[0].set_title("Total number changes: mean +/- std")

    plot_mean_with_std(axes[1], aggregate["normal_number_changes_per_step"], label="Normal", color="steelblue")
    plot_mean_with_std(
        axes[1],
        aggregate["freeze_number_changes_per_step"],
        label=freeze_label,
        color="coral",
    )
    axes[1].set_xlabel("Recursive step")
    axes[1].set_ylabel("Average number changes")
    axes[1].set_title("Number changes per step (mean +/- std)")
    axes[1].legend()
...
def plot_metric_bar(ax, data: dict, *, normal_name: str, freeze_name: str, ylabel: str, title: str):
    ...
    ax.set_xticks(x, ["Normal", freeze_name.replace("Total number changes: ", "").replace("Mean number changes per step: ", "")])
...
def plot_aggregate_json(input_path: Path, data: dict, output_path: Path):
    ...
    plot_metric_bar(
        axes[0],
        data,
        normal_name="Total number changes: normal",
        freeze_name=f"Total number changes: {freeze_label}",
        ylabel="Total number changes",
        title="Total number changes across runs",
    )
    plot_metric_bar(
        axes[1],
        data,
        normal_name="Mean number changes per step: normal",
        freeze_name=f"Mean number changes per step: {freeze_label}",
        ylabel="Mean number changes per step",
        title="Mean number changes per step across runs",
    )
    plt.tight_layout()
    plt.savefig(output_path, dpi=500)
    plt.close()
...
def write_aggregate_from_manifest(manifest_path: Path, *, make_plots: bool) -> list[Path]:
    ...
    artifacts = [
        ("sudoku_freeze_zH_results.json", "sudoku_freeze_zH_aggregate_5runs"),
        ("sudoku_freeze_zL_results.json", "sudoku_freeze_zL_aggregate_5runs"),
    ]
...
def main():
    ...
    parser.add_argument("json_paths", nargs="*", type=Path, help="Sudoku freeze result or aggregate JSON files")
