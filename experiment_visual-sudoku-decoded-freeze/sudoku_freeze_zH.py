...
If we stop updating z_H and let only z_L recurse, does the mask still shift?
...
Run from AIR_code root: python experiment_visual-sudoku-decoded-freeze/sudoku_freeze_zH.py
"""
...
def main():
    print(f"Running normal + freeze z_H over first {NUM_QUESTIONS} eval questions (streaming).")
    ...
    metric_order = [
        ...
        "Total mask flips: freeze z_H",
        "Delta total flips (freeze - normal)",
        "Total number changes: normal",
        "Total number changes: freeze z_H",
        ...
        "Mean number changes per step: freeze z_H",
        "Exact accuracy: normal",
        "Exact accuracy: freeze z_H",
    ]
    ...
    for _batch_index in range(NUM_QUESTIONS):
        ...
        freeze_traces = run_act_with_tracing(air, inner, batch, freeze_z_H=True)

        normal_steps = [step["pred_L"] for step in flatten_recursive_traces(normal_traces)]
        freeze_steps = [step["pred_L"] for step in flatten_recursive_traces(freeze_traces)]
        ...
        scalar_stats["Total mask flips: freeze z_H"].update(total_freeze_flips)
        ...
        scalar_stats["Total number changes: freeze z_H"].update(total_freeze_changes)
        ...
        scalar_stats["Mean flips per step: freeze z_H"].update(mean_freeze_flips)
        scalar_stats["Mean number changes per step: normal"].update(mean_normal_changes)
        scalar_stats["Mean number changes per step: freeze z_H"].update(mean_freeze_changes)
        scalar_stats["Exact accuracy: normal"].update(exact_normal)
        scalar_stats["Exact accuracy: freeze z_H"].update(exact_freeze)
    ...
    write_markdown_table(
        os.path.join(OUT_DIR, "sudoku_freeze_zH_summary.md"),
        table_headers,
        table_rows,
        title="Sudoku Freeze z_H Summary",
    )
    write_json(
        os.path.join(OUT_DIR, "sudoku_freeze_zH_results.json"),
        {
