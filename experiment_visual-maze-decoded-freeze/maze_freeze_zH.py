#!/usr/bin/env python3
"""
Freeze z_H ablation for Maze.

If we stop updating z_H and let only z_L recurse, does the solution-path mask
still shift?

This tests whether mask motion is driven by H feedback or by intrinsic L dynamics.

Run from AIR_code root: python experiment_visual-maze-decoded-freeze/maze_freeze_zH.py
"""
...
def analyze_question_pair(normal_run: dict, freeze_run: dict) -> dict:
    normal_steps = [step["pred_L"] for step in flatten_recursive_traces(normal_run["act_step_traces"])]
    freeze_steps = [step["pred_L"] for step in flatten_recursive_traces(freeze_run["act_step_traces"])]
...
def main():
    ...
    print(f"(2) Freeze z_H run across first {NUM_QUESTIONS} eval questions")
    ...
    freeze_per_step_stats = aggregate_series(
        [result["freeze_zH"]["per_step_flips"] for result in paired_results]
    )
    ...
    freeze_content_stats = aggregate_series(
        [result["freeze_zH"]["per_step_content_changes"] for result in paired_results]
    )
    ...
    table_rows = metrics_table_rows(
        {
            ...
            "Total solution-mask flips: freeze z_H": [
                result["freeze_zH"]["total_mask_flips"] for result in paired_results
            ],
            "Delta total flips (freeze - normal)": [
                result["freeze_zH"]["total_mask_flips"] - result["normal"]["total_mask_flips"]
                for result in paired_results
            ],
            "Total color changes: normal": [result["normal"]["total_color_changes"] for result in paired_results],
            "Total color changes: freeze z_H": [result["freeze_zH"]["total_color_changes"] for result in paired_results],
            "Delta total color changes (freeze - normal)": [
                result["freeze_zH"]["total_color_changes"] - result["normal"]["total_color_changes"]
                for result in paired_results
            ],
            "Mean flips per step: normal": [result["normal"]["mean_flips_per_step"] for result in paired_results],
            "Mean flips per step: freeze z_H": [
                result["freeze_zH"]["mean_flips_per_step"] for result in paired_results
            ],
            "Mean color changes per step: normal": [result["normal"]["mean_color_changes_per_step"] for result in paired_results],
            "Mean color changes per step: freeze z_H": [
                result["freeze_zH"]["mean_color_changes_per_step"] for result in paired_results
            ],
            "Exact accuracy: normal": [result["normal"]["exact_accuracy"] for result in paired_results],
            "Exact accuracy: freeze z_H": [result["freeze_zH"]["exact_accuracy"] for result in paired_results],
        }
    )
    table_text = markdown_table(table_headers, table_rows)
    write_markdown_table(
        os.path.join(OUT_DIR, "maze_freeze_zH_summary.md"),
        table_headers,
        table_rows,
        title="Maze Freeze z_H Summary",
    )
    write_json(
        os.path.join(OUT_DIR, "maze_freeze_zH_results.json"),
        {
